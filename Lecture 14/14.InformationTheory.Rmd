---
title: "Lecture 14: Information theory"
output: pdf_document
fontsize: 12pt 
...
library(extrafont)
loadfonts()
setwd("~/Dropbox/BayesClass/2018 Class/Lecture 14")
...
This is the definition of *information*: the reduction in uncertainty derived from learning the outcome. For this definition to be useful, we need a way to quantify the uncertainty intrinsic to a probability distribution. 
...
Note that this equation blows up when $p_i=0$ because $\log{0}=-\infty$. Unacceptable! Fortunately, we can use L'H\^{o}pital's rule: $\lim_{p_i \to 0} p_i\log{p_i} = 0$ and assume that $0\log{0}=0$. 
