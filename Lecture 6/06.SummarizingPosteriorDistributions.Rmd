---
title: 'Lecture 6: Summarizing posterior distributions'
...
library(extrafont)
loadfonts()
setwd("~/Dropbox/BayesClass/2018 Class/Lecture 6")
...
```
```{r engine = 'cat', engine.opts = list(file = "06.binomialMode.stan", lang = "stan"), echo=FALSE}
data {
  int<lower=0> nObs;  // Total number of observations
  int<lower=0> N[nObs];
  int<lower=0> obs[nObs];   // obs as scalar
...
Because equal probability mass is in the upper and lower tails, it excludes parameter values > $\approx$ `r round(quantile(p,0.75),2)` despite their having a high probability. 
...
\begin{equation}
  p(\tilde{y}|y) \sim \int p(\tilde{y}|p)p(p|y) dp
\end{equation}
...
```{r engine = 'cat', engine.opts = list(file = "06.binomialGQ.stan", lang = "stan"), echo=FALSE}
data {
  int<lower=0> nObs;  // Total number of observations
  int<lower=0> N[nObs];
  int<lower=0> obs[nObs];   // obs as scalar
